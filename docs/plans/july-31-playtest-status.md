...
| Jul 15 | Packaged networking | YELLOW | Freshness-aware candidate verifier PASS: executable/map/cue, handshake, normal/impaired roster, reconnect-attempt, and death/wisp/revive gates | Deliberate visual acceptance and second-machine direct-IP pending |
...
| BB-PT-017 | P1 | Death/Wisp | Packaged lethal-damage-to-wisp lifecycle lacked deterministic evidence | Codex | Resolved for core lifecycle; visual bars/proximity priority and persistent-ability death cleanup remain manual | `Saved/Logs/PackagedDeathWisp/20260712-205037`: GAS depletion, wisp possession, capped healing revive, server hunter restore, victim-client repossession PASS; 0 critical/cue overflow |
...
- Current package: source-toolchain client and server archives rebuilt at 20:44 after death/wisp/revive automation; packaged dedicated handshake 2/2 PASS at `Saved/Logs/PackagedHandshake/20260712-204438`; interactive combat and second-machine evidence remain pending
- Candidate verifier: current result PASS for source freshness, client/server executables, cooked TestMap, Hudson cues 17/17, handshake 2/2, normal and impaired full-roster smoke 6/6, reconnect attempt, death/wisp/revive, and zero critical packaged-log findings; summary at `Builds/PlaytestCandidateVerification.txt`
...
- Network impairment: repeatable packaged gate applies and verifies `PktLag=100` plus `PktLoss=2` on server and clients; activation lifecycle PASS for hunters 1-6 at `Saved/Logs/PackagedNetworkImpairment/20260712-204709`, `20260712-204759`, and `20260712-204848`; manual sustained-fire/zone-overlap visual checks remain open
- Death/wisp automation: opt-in `-BBDeathSmoke` plus `TestPackagedDeathWispSmoke.ps1` now covers shared GAS lethal damage, HealthSet depletion, authoritative/client wisp possession, server-timed capped healing, revive at 50% health, and victim-client hunter repossession; main-package PASS at `Saved/Logs/PackagedDeathWisp/20260712-205037`
